...
{
    public partial class RimportSPSSDatabase : IDataSource 
    {
        /// <summary>
        ...
        public System.Data.IDataReader GetDataTableReader(string pSQL)
        {
            ...
            {
                Logger.Log("Error RimportSPSSDatabase.IDataSource.GetDataTableReader:\n" + e.ToString());
            }
            return result;*/

            string tableName = pSQL.Substring(pSQL.ToLower().IndexOf(" from ") + 6).Split(' ')[0].Replace("`", "").Replace("'", "").Replace(";", "").Replace("[", "").Replace("]", "");

            return null;// new RimportSPSSWrapper(connectionString).GetDataTableAsync(tableName).Result.CreateDataReader();
        }
        ...
        public bool ExecuteSQL(string pSQL)
        {
            ...
            {
                Logger.Log("Error RimportSPSSDatabase.IDataSource.Execute:\n" + e.ToString());
            }
            return result;
        }
        ...
        public override bool CheckDatabaseExistance(string connectionString, string pTableName, bool pIsConnectionString = false)
        {
            ...
            return new RimportSPSSWrapper(connectionString).GetDatabaseNames().Contains(databaseName);
        }
    }
}
