...
using Epi.Data.REDCap.Wrappers;
...
namespace Epi.Data.REDCap
{
    public partial class REDCapDataSource : DbDriverBase
    {
        ...
        {
            get { return "REDCap Database: " + this.DbName; }
        }
        ...
        public override void CreateTable(string tableName, List<TableColumn> columns)
        {
            ...
            /*new REDCapWrapper(connectionString).CreateCollection(tableName);
            ...
            REDCapCommand command = new REDCapCommand(sb.ToString());
            REDCapConnection conn = new REDCapConnection(this.ConnectionString);
        ...
        // Add columns to a REDCap Database
        ...
        public override string ConnectionString
        {
            ...
            set
            {
                ...
                {
                    this.DbName = new REDCapWrapper(connectionString).GetOrgName();
                }
        ...
        public static string BuildConnectionString(string filePath, string password)
        {
            ...
            // "database=myDB;server=/var/lib/REDCap/REDCap.sock;user id=myUser; pwd=myPass"; 
        ...
        public static string BuildDefaultConnectionString(string databaseName)
        {
            ...
            //This method may need to be deprecated
            //Linux: return BuildConnectionString("database=" + databaseName + ";server=//var/lib/REDCap/REDCap.sock;user id=myUser; pwd=myPass","");
            return null;
        ...
        public override string DataSource
        {
            get
            {
                ...
                {
                    return new REDCapWrapper(connectionString).GetOrgName();
                }
            }
        }
        ...
        public override DataTable Select(Query selectQuery, DataTable dataTable)
        {
            ...
            REDCapDataAdapter adapter = new REDCapDataAdapter();
            adapter.SelectCommand = (REDCapCommand)GetCommand(selectQuery.SqlStatement, connection, selectQuery.Parameters);
            ...
            {
                dataTable.Merge(new REDCapWrapper(connectionString).GetFirstDataRow(tableName));
            }
            else
            {
                dataTable.Merge(new REDCapWrapper(connectionString).GetDataTableAsync(tableName).Result);
            }
        ...
        {
            return new REDCapWrapper(connectionString).GetTableNames().Contains(tableName);
        }

        /// <summary>
        /// Gets column schema information about an REDCap table
        /// </summary>
        /// <param name="tableName">Name of the table</param>
        /// <returns>DataTable with schema information</returns>
        ...
        /// <summary>
        /// Gets Primary_Keys schema information about a REDCap table
        /// </summary>
        /// <param name="tableName">Name of the table</param>
        /// <returns>DataTable with schema information</returns>
        ...
        public override List<string> GetTableNames()
        {
            ...
            return new REDCapWrapper(connectionString).GetTableNames();

        }

        /// <summary>
        /// Create REDCap query object
        /// </summary>
        ...
        /// <summary>
        /// Inserts the string in escape characters. [] for SQL server and `` for REDCap etc.
        /// </summary>
